...
{
	unsigned int i;
	for (i=0;i<timedelay;i++) { _delay_ms(0.5); }
}
...
int main(void)
{
	...
	//************************************************************************
	//If we get here, then the bootloader delay occurs
	...
	//Prevents GCC from generating exit return code
...
{
	char	theChar = 0x30 + ((theByte >> 4) & 0x0f);

	if (theChar > 0x39) { theChar += 7; }
	sendchar(theChar );
...
static void	DumpHex(unsigned char dumpWhat, unsigned long startAddress, unsigned char numRows)
{
	...
	while (numRows > 0)
	{
		if (myAddressPointer > 0x10000) { PrintHexByte((myAddressPointer >> 16) & 0x00ff); }
		...
		for (ii=0; ii<16; ii++)
		{
			switch(dumpWhat)
			{
				...
				case kDUMP_EEPROM: theValue = eeprom_read_byte((uint8_t *)(uint16_t)myAddressPointer); break;
				case kDUMP_RAM:    theValue = ramPtr[myAddressPointer];                                break;
			}

			PrintHexByte(theValue);
			sendchar(0x20);
			if ((theValue >= 0x20) && (theValue < 0x7f)) { asciiDump[ii % 16] = theValue; }
			else                                         { asciiDump[ii % 16] = '.';      }
...
static void	EEPROMtest(void)
{
...
#endif
	{
		...
		if (theChar == 0) { PrintFromPROGMEM(gTextMsg_SPACE, 0); }
		else              { sendchar(theChar);                   }
		ii++;
	}
...
#endif
	{
		...
		if (theEEPROMchar == 0) { PrintFromPROGMEM(gTextMsg_SPACE, 0); }
		else                    { sendchar(theEEPROMchar);             }
		
		if (theEEPROMchar != theChar) { errorCount++; }
		ii++;
	}
	PrintNewLine();
	PrintNewLine();
...
static void VectorDisplay(void)
{
	...
	while (vectorIndex < kInterruptVectorCount)
	{
		...
		if (word1 == 0xffff) { PrintFromPROGMEM(gTextMsg_noVector, 0); }
...
static void	AVR_PortOutput(void)
{
	...
	if ((portLetter >= 'A') && (portLetter <= 'Z'))
	{
		...
		if (getCharFlag) { recchar(); }
	}
	else { PrintFromPROGMEMln(gTextMsg_MustBeLetter, 0); }
}
...
static void	RunMonitor(void)
{
	...
	while (keepGoing)
	{
		...
		theChar	=	recchar();
		if (theChar >= 0x60) { theChar = theChar & 0x5F; }
		if (theChar >= 0x20)
		{
			sendchar(theChar);
			sendchar(0x20);
		...
		switch(theChar)
		{
			...
			case 'E':
				...
				gEepromIndex	+=	256;
				if (gEepromIndex > E2END) { gEepromIndex = 0; }
				break;
