...
#include "libcm.h"
	...
	#define LIDISPLAY_UPDATE_RATE_MILLIS 40		// One element is updated each time
		...
		// Initializing to absurd numbers so that on first run they will be updated on screen
		static uint8_t  LiDisplayPackVoltageActual_onScreen = 100; //JTS: these are already static because they aren't called in a function
		...
		static uint8_t	LiDisplayTemp_onScreen = 0; //JTS: This isn't an 'absurd' number
		static uint16_t	LiDisplayAverageCellVoltage = 0; //JTS: This isn't an 'absurd' number
		static uint8_t maxElementId = 8; //JTS: This isn't an 'absurd' number
		...
		const String attrMap[3] = { "txt", "val", "pic" };
		const String fanSpeedDisplay[3] = {"FAN OFF", "FAN LOW", "FAN HIGH"};
	#endif

	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	uint8_t LiDisplay_bytesAvailableForWrite(void) { return Serial1.availableForWrite(); }
	uint8_t LiDisplay_bytesAvailableToRead()       { return Serial1.available();         }

	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

	void    LiDisplay_writeByte(uint8_t data) { Serial1.write(data);   }
	uint8_t LiDisplay_readByte(void)          { return Serial1.read(); }
	...
	void LiDisplay_setPageNumber(uint8_t page)
	{
		...
		#else
			if(page){;} //prevent compiler warning
		#endif
	}
	...
	void LiDisplay_updateNumericVal(uint8_t page, String elementName, uint8_t elementAttrIndex, String value) {
		#ifdef LIDISPLAY_CONNECTED
			static String LiDisplay_Number_Str; //JTS: Why is this static?  Next line always calculates it again

			LiDisplay_Number_Str = "page" + String(page) + "." + String(elementName) + "." + attrMap[elementAttrIndex] + "=" + value;

			Serial1.print(LiDisplay_Number_Str); //JTS: Use LiDisplay_writeByte() instead
			Serial1.write(0xFF);
			Serial1.write(0xFF);
			Serial1.write(0xFF);
		#else
			if(page){;} //prevent compile warnings
			if(elementName){;}
			if(elementAttrIndex){;}
			if(value){;}
		#endif
	...
	void LiDisplay_updateStringVal(uint8_t page, String elementName, uint8_t elementAttrIndex, String value) {
		#ifdef LIDISPLAY_CONNECTED
			static String LiDisplay_String_Str; //JTS: Why is this static?  Next line always calculates it again
	...
	void LiDisplay_updateNextCellValue(void) {
		#ifdef LIDISPLAY_CONNECTED
	...
	void processLiDisplayRequest(void)
	{
			...
			static uint8_t LiDisplayWaitingForCommand = 0;
	...
	void LiDisplay_handler(void)
	{
			...
			if(digitalRead(PIN_HMI_EN) == 1)
			{
				...
				processLiDisplayRequest();

				LiDisplay_calculateCorrectPage();

				if (LiDisplayPowerOffPending) {
					if (LiDisplayOnGridChargerConnected) {
						// Grid Charger plugged in before relay in IPU compartment clicked off, or just after, but before splash screen complete.
